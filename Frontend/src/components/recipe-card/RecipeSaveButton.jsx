...
export default function RecipeSaveButton({ recipe }) {
  ...
  if (recipe.ownerId === currentUserId) {
    return <span className="recipe-owner" role="img" aria-label="Ditt recept" title="Ditt recept">
  ...
  return (
    <button type="button" className="recipe-save" aria-pressed={saved}
      aria-label={`Spara ${recipe.name}`} title={saved ? "Ta bort sparat recept" : "Spara recept"}
      onClick={() => toggleSaved(recipe.id)}>
      <SaveIcon size={20} fill={saved ? "currentColor" : "none"} aria-hidden="true" />
    </button>
  );
}
